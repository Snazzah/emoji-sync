...
export class SequentialBucket {
  ...
  async #execute(request: Request, attempts = 0): Promise<any> {
    ...
    if (this.#handler.manager?.listenerCount('rawREST'))
      this.#handler.manager.emit('rawREST', {
  ...
  #handle(request: Request, response: Response, latency: number) {
    ...
    if (hash) {
      if (this.#hash !== hash) {
        ...
        this.#handler.manager?.emit('debug', `Updated bucket hash (${this.#hash}) to ${hash}`);
      } else {
